...
function createCard() {
  for (let i = 0; i < photographers.length; i += 1) {
    ...
    const photographerTagArr = [];
    for (let j = 0; j < tagsData.length; j += 1) {
      const photographerTag = document.createTextNode(tagsData[j]);
      tagBox.appendChild(photographerTag);
      photographerTagArr.push(photographerTag);
    }
    for (let k = 0; k < photographerTagArr.length; k += 1) {
      const prout = photographerTagArr[k];
      prout.className = 'tag';
      console.log(prout);
    };
...
navTag.classList.add('navigation-item');
navTag.addEventListener('click', getRelevantData);
